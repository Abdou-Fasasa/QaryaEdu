...
function normalize_state($state): array {
    ...
    return [
        ...
        'supportThreads' => isset($state['supportThreads']) && is_array($state['supportThreads']) ? array_values($state['supportThreads']) : [],
...
function merge_support_threads(array $current, array $incoming): array {
    $records = [];
    foreach ([$current, $incoming] as $items) {
        foreach ($items as $item) {
            if (!is_array($item)) continue;
            $key = trim((string) ($item['id'] ?? ($item['email'] ?? '')));
            if ($key === '') continue;
            $existing = $records[$key] ?? null;
            if ($existing === null || timestamp_from_values($item['updatedAt'] ?? '', $item['createdAt'] ?? '') >= timestamp_from_values($existing['updatedAt'] ?? '', $existing['createdAt'] ?? '')) {
                $records[$key] = $item;
            }
        }
    }
...
$mergedState = [
    ...
    'supportThreads' => merge_support_threads($currentState['supportThreads'], $incomingState['supportThreads']),
