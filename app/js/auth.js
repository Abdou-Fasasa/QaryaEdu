(() => {
    ...
    function shouldKeepUserAfterStudentCleanup(user = {}) {
        // Accounts are only removed through the explicit administrator delete
        // operation. Never silently remove students during synchronization.
        return Boolean(user && user.storageKey);
    }
    ...
    function applyEidGiftToStudents(users = getAllUsersRaw()) {
        // Balances are changed only by an explicit administrator action or a
        // recorded exam reward. Loading a page must never grant money silently.
        return { changed: false, users: mergeUsers(users) };
    }
