document.addEventListener('DOMContentLoaded', () => {
    ...
    // نظام إغلاق التسجيل: تم تمكين التسجيل بناءً على طلب الإدارة
    /* 
    if (registerForm) {
        displayError("تنبيه: باب التسجيل مغلق حالياً بقرار من الإدارة. لا يمكن استقبال طلبات جديدة في الوقت الحالي.");
    ...
    */
    ...
    function displaySuccess(application, replacedExisting) {
        ...
        registerResultDiv.innerHTML = `
            ...
            <div class="admin-edit-grid" style="margin-top:1rem;">
                <div class="admin-field">
                    <label>بريد الطالب</label>
                    <input class="form-control" id="student-email-preview" type="text" value="${application.studentEmail || ''}" readonly />
                </div>
                <div class="admin-field">
                    <label>كلمة المرور</label>
                    <input class="form-control" id="student-password-preview" type="text" value="${application.studentPassword || ''}" readonly />
                </div>
                <div class="admin-field">
                    <label>كود القائد</label>
                    <input class="form-control" type="text" value="${application.leaderCode || ''}" readonly />
                </div>
            </div>
            <div class="result-actions wrap-actions">
                ...
                <button type="button" class="btn-secondary" data-copy-target="#student-email-preview" data-copy-label="نسخ البريد">نسخ البريد</button>
                <button type="button" class="btn-secondary" data-copy-target="#student-password-preview" data-copy-label="نسخ كلمة المرور">نسخ كلمة المرور</button>
                ...
                <a href="../login.html" class="btn-ghost">الدخول للمنصة</a>
            </div>
            <p class="result-note">${noteText} وتم إنشاء حساب دخول عادي للطالب تلقائيًا.</p>
        `;
        registerResultDiv.style.display = 'block';
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async function (event) {
            ...
            const existingStudentAccount = authApi?.getUserByNationalId?.(nationalId);
            const studentCredentials = existingStudentAccount
                ? { email: existingStudentAccount.email, password: existingStudentAccount.password }
                : (authApi?.buildStudentCredentials?.({
                    nationalId,
                    requestId,
                    name: String(formData.get('name') || '').trim()
                }) || {
                    email: `student.${nationalId}@qarya.edu`,
                    password: nationalId.slice(-6) || requestId
                });
            const application = {
                requestId,
                nationalId,
                ...
                studentPassword: studentCredentials.password,
            ...
            if (platformStore?.addNotification) {
                platformStore.addNotification({
                    ...
                    body: `تم تسجيل طلب جديد باسم ${application.name} وهو الآن قيد المراجعة، وتم إنشاء حساب دخول للطالب.`,
                    type: 'application',
